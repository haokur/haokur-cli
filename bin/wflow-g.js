...
// 抽取命令
let [, , order, ...createNames] = process.argv;

let { orders: recognizableOrders } = wflowConf.g;
...
function generateAndWriteFile(order, createDir, createName) {
    ...
    Object.keys(orderTargetConf).forEach(key => {
        let _filePath = path.resolve(
            createDir,
            orderTargetConf[key].replace(/{{createName}}/g, createName)
        );
        isExistFile$()
            .catch(() => {
                return getOrderTemplate(order, key, createName).then(tplStr => {
                    // 文件不存在,则开始创建文件
